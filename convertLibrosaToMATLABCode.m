...
%% Requirements
%% 
% * <https://www.mathworks.com/ MATLAB®> R2023b or later
...
istftOut2 = librosa.istft(stftOut2, ...
    FFTLength=512, ...
    HopLength=160, ...
    WindowLength=512, ...
    Window="hann", ...
...
melOut2 = librosa.mel(SampleRate=fs, ...
    FFTLength=512, ...
    NumBands=50, ...
...
melSpectrogramOut2 = librosa.melSpectrogram(samples, ...
    SampleRate=fs, ...
    FFTLength=512, ...
    NumBands=50,...
    Center=false, ...
...
librosa.melSpectrogram(samples, ...
    SampleRate=fs, ...
    FFTLength=512, ...
    NumBands=50, ...
    Center=false, ...
...
% model import-export functionality>. In this example, you use |importNetworkFromONNX|. 
% The function imports a version of the network that was saved to the  Open Neural 
% Network Exchange (ONNX) format. To see how the PyTorch model can be saved to 
% an ONNX format, refer to <./PythonCode\convertModelToONNX.py convertModelToONNX.py>.
...
spect = librosa.melSpectrogram(samples, ...
    SampleRate=fs, ...
    FFTLength=512, ...
    NumBands=50, ...
    Center=false, ...
